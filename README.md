...
Weight quantization from primitives: integer 8-bit (tensor / channel / row / zero-point), **NF4** for QLoRA/QDoRA frozen bases, and 4-bit E2M1 with hierarchical scales (NVFP4).
...
- Dettmers et al., *QLoRA* — **NF4** (NormalFloat4) codebook + block absmax; QDoRA freezes the same NF4 base under DoRA.
...
`NF4Weight.dequant()` is the on-the-fly path adapters multiply against; codes stay frozen.
